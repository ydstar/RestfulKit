 ...
 *  4.这个ProxyCall对象就是ICall<?>的实现类对象,然后在invoke方法内返回,因为动态代理这个invoke要返回的值就是传入的接口中的被调用的方法的返回值
 *   所以必须返回ICall<?>的实现类对象
 *  5.ProxyCall本身是真正的发起网络请求的类,因为ProxyCall对象是用Scheduler的newCall方法创建的,然后方法内是用的callFactory创建的
 *  6.这个callFactory是在外界ApiFactory类中创建IRestful的时候传递进来的
 */
class IRestful constructor(val baseUrl: String, callFactory: ICall.Factory) {

    //解析的方法map
    private var mMethodParserMap = ConcurrentHashMap<Method, MethodParser>()

    //自定义拦截器list
    private var mInterceptorList = mutableListOf<IInterceptor>()
    ...
    fun <T> create(service:Class<T>):T{
        ...
        val t = Proxy.newProxyInstance(service.classLoader,arrayOf<Class<*>>(service),object : InvocationHandler {
             ...
             * method:调用的方法(listCities方法)
             * args:调用的方法的入参listCities(@Path("province") province: Int,@Filed("page") page: Int)
             * 返回值:就是定义接口中方法的返回值ICall<JsonObject>
             */
            override fun invoke(proxy: Any, method: Method, args: Array<out Any>?): Any {

                var methodParser = mMethodParserMap[method]
                if (methodParser == null) {
                    //解析函数上的所有注解并封装成IRequest对象
                    methodParser = MethodParser.parse(baseUrl, method)
                    mMethodParserMap[method] = methodParser
                }

                //解析完后得到IRequest对象,还需要封装成ICall接口的实现类对象,
